...
namespace MiepMiep
{
	...
	MM_TS ESendCallResult priv_send_rpc(INetwork& nw, const char* rpcName, BinSerializer& bs, const IEndpoint* specific, bool exclude, bool buffer, bool relay, byte channel, IDeliveryTrace* trace)
	{
		BinSerializer bsCopy;
		__CHECKEDSR( bs.length() < UINT16_MAX );
