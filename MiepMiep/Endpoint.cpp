...
namespace MiepMiep
{
	...
	void AddressList::addAddress( const IAddress& addr, const MetaData& metaData )
	{
	...
	void AddressList::removeAddress( const IAddress* addr )
	{
		if ( !addr ) return;
		m_Addresses.erase( std::find_if( m_Addresses.begin(), m_Addresses.end(), [&]( auto& adp ) { return *adp->m_Addr == *addr; } ) );
	}

	bool AddressList::readOrWrite( BinSerializer& bs, bool _write )
	{
		if ( _write )
		{
	...
	MetaData AddressList::metaData( u32 idx ) const
	{
	...
	sptr<const IAddress> AddressList::address( u32 idx ) const
	{
		if ( idx < m_Addresses.size() )
		{
	...
	sptr<Endpoint> Endpoint::createEmpty()
	{
		return reserve_sp<Endpoint>( MM_FL );
	}

	sptr<Endpoint> Endpoint::createSource( const ISocket& sock, i32* errOut )
	{
		if ( errOut ) *errOut = 0;
